...
class Skeletonize():
    ...
    def calcEndSimilarity(self, end_dict_1, end_dict_2, skel, flag='end_pair'):
        ...
        if self.if_debug:
            ...
            if flag == 'int_pair':
                back = back[max(0, ((point1[0] + point2[0])//2 - 20*self.kernel_size)):min(((point1[0] + point2[0])//2 + 20*self.kernel_size), self.IMG_H-1),
                            max(0, ((point1[1] + point2[1])//2 - 20*self.kernel_size)):min(((point1[1] + point2[1])//2 + 20*self.kernel_size), self.IMG_W-1)]
