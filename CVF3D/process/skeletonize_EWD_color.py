...
class Skeletonize():
    ...
    def run(self, mask, density=7, source_img=None):
    ...
    def calcEndSimilarity(self, end_dict_1, end_dict_2, skel, flag='end_pair'):
        ...
        CM = 0.01 * CE + CD + CC + 0.2 * CW + 0.1 * CH
        if self.if_debug:
            back = cv2.cvtColor(skel, cv2.COLOR_GRAY2BGR)
            back = ~back
            ...
            if flag == 'int_pair':
                back = back[max(0, ((point1[0] + point2[0])//2 - 20*self.kernel_size)):min(360, ((point1[0] + point2[0])//2 + 20*self.kernel_size)),
                            max(0, ((point1[1] + point2[1])//2 - 20*self.kernel_size)):min(640, ((point1[1] + point2[1])//2 + 20*self.kernel_size))]
    ...
    def handleIntersections(self, skel, ends_dict_rf, ints_dict_m):
        ...
        for i, int_dict in ints_dict_m.items():
            ...
            if int_end_num == 1:
                ...
                end_pairs += self.handleNCross(skel, ends_dict_rf, int_dict)
        for k in to_delete:
            del ints_dict_m[k]
        return end_pairs
    ...
    def handleNCross(self, skel, ends_dict_rf, int_dict):
        print('handleNCross')
        end_pairs = []
        int_dict_ends = int_dict['int_ends']
        ...
        for i in range(cross_num - 1):
            for j in range(i+1, cross_num):
                index_l.append(str(i)+str(j))
        CM = {}
        for index in index_l:
            ind1 = int(index[0])
            ind2 = int(index[1])
            CM[index] = self.calcEndSimilarity(ends_dict_rf[int_dict_ends[ind1]], ends_dict_rf[int_dict_ends[ind2]],
                                               skel, 'int_pair')
        CM_l = [v for v in CM.values()]
        end = [str(k) for k in range(cross_num)]
        end_already_in = []
